 ...
 */
public class SearchList  extends BaseForm{
    
     public SearchList(Resources res , String r) {
        NewsfeedForm nf = new NewsfeedForm(res);
      //  AjouterEvenement al = new AjouterEvenement(res);

        ArrayList<Evenement> evenements = null;

        ArrayList<Evenement> list =  ServiceEvenement.getInstance().SearchEvenement(r);
        evenements = list;
...
//        search.setUnselectedStyle(textFieldStyle);
//        
...
//        ajouterButton.addActionListener(e -> ServiceEvenement.getInstance().SearchEvenement(r));
        // Handle adding a new livreur here

      //  buttonsContainer.add(ajouterButton);
     // EvenementList el = new EvenementList(res);

        Button retourButton = new Button("Retour");
        retourButton.addActionListener(e -> {
           
                boolean confirmed = Dialog.show("Confirmation", "Are you sure ", "Yes", "No");
                if (confirmed) {
        ...
        for (Evenement evenement : evenements) {
           ...
           Label description = new Label("Description: " + evenement.getDescription());
            Label datedebut = new Label("Date Debut: " + evenement.getDateString1());
